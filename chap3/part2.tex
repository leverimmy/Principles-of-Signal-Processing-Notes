...
\subsubsection{离散时间傅里叶变换的定义}

\begin{definition}
    对于连续时间信号 $f(t)$ 进行以 $T_s$ 为时间间隔的采样，得到序列 $f(nT_s)$，
    其中 $n \in \set{Z}$。那么，$f(nT_s)$ 的\bd{离散时间傅里叶变换}（DTFT）
    定义为
    \begin{align*}
...
\begin{definition}
    ...
    \end{align*}
    此即为\bd{逆离散时间傅里叶变换}（IDTFT）。
\end{definition}

\subsubsection{数字信号与数字频谱}

由于采样间隔为固定的值，因此，不妨设 $T_s = 1$。这就是在
...
\begin{solution}
    \begin{enumerate}[label=(\arabic*)]
        \item $\DTFT{x(n) * x^*(-n)} = \DTFT{x(n)} * \DTFT{x^*(-n)}
