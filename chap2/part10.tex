...
\begin{definition}[欠采样]
    ...
    \begin{enumerate}
        \item 频谱混叠的情况下时域信号变了，但采样点处取值不变。
            ...
            将具有相同的值，即 $x_r(nT) = x(nT)$。
        \item 工程应用时，如果采样频率 $\omega_s = 2\omega_M$，则
            不足以从样本中恢复原信号。

            例如 $x(t) = \cos(\omega_0 t + \varphi)$ 在 $\omega_s = 2\pi/T = 2\omega_0$ 时条件下
            进行采样，则
            \begin{align*}
                x(nT) & = \cos(\omega_0 nT + \varphi) \\
                & = \cos\varphi\cos(\omega_0 nT) - \sin\varphi\sin(\omega_0 nT) \\
                & = \cos\varphi\cos n\omega_0 T.
            \end{align*}
